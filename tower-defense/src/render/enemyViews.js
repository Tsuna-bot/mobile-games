...
export class EnemyViews {
  ...
  createView(type) {
    ...
    return { type, root, model, material, bar, fill, fillMaterial, flash: 0, spawn: 0, spin: Math.random() * Math.PI * 2, px: 0, pz: 0 };
  }
  ...
  update(dt, time, camera) {
    ...
    for (const view of this.active) {
      ...
      const bob = Math.sin(time * 3 + enemy.id) * 0.06;
      view.root.position.set(view.px, CONFIG.world.enemyHover + bob, view.pz);

      view.spin += dt * (enemy.def.id === 'runner' ? 5 : 1.6);
      ...
      if (enemy.slowFactor < 1) emissive.copy(FROST_TINT).multiplyScalar(0.45);
